...
//
//	vec4 position = gl_Position;//gl_ModelViewMatrix * gl_Vertex;
//
//	vec3 viewDirection = normalize(cameraPosition - vec3(gl_Vertex));
//
//	float dotProd = dot(viewDirection, surfaceNormal);
//
...
//	testCol.b = 0;
	 ...
	 */
	vec3 lightDirection = normalize(gl_LightSource[lightNumber].position.xyz - vertexPosition);
	...
	// Adds ambient color to the color so even the darkest part equals ambientColour.
	...
	if (diffuseLightIntensity != 0)
	{
		...
		vec3 col = normalize(specColor);

		varyingColour.rgb += col * vec3(fspecular, fspecular, fspecular) * vec3(intensity, intensity, intensity);
	}
