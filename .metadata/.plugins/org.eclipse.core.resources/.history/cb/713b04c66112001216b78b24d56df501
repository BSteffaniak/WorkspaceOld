...
import static net.foxycorndog.jdooglandroid.components.ImageTextField.Margin.*;
...
public class ImageTextField extends ImageButton implements KeyListener, ActionListener
{
	...
	public void render()
	{
		GL.beginManipulation();
		{
			...
			boolean difSize = getWidth() != getTextureWidth() || getHeight() != getTextureHeight();
			
			if (difSize)
			{
				super.render(getWidth() / (float)getTextureWidth(), getHeight() / (float)getTextureHeight(), 1);
			}
			else
			{
				super.render();
			...
			float offsetX = 0;//((getScreenWidth() - lm - rm) - Frame.getFont().getLegitWidth(fieldText + "_") * getScaleWidth());
